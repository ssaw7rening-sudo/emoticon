...
function hairFurPrecisionV2() {
  return {
    name: 'hair-fur-fine-detail-precision-v2',
    enforce: 'post',
    transform(code, id) {
      ...
      if (!transformed.includes(assessAnchor)) {
        throw new Error('[hair-fur-v2] Quality-assessment anchor was not found')
      }
      if (!transformed.includes('HYBRID_ORIGINAL_RESOLUTION_EDGE_REFINEMENT_V1')) {
        throw new Error('[hair-fur-v2] Hybrid edge refinement must run first')
      }

      const helper = `// HAIR_FUR_FINE_DETAIL_PRECISION_V2
...
`
      ...
      if (!transformed.includes(downloadAnchor)) {
        throw new Error('[hair-fur-v2] Download handler anchor was not found')
      }
...
`
      ...
      if (!transformed.includes(messageAnchor)) {
        throw new Error('[hair-fur-v2] Precision-message UI anchor was not found')
      }
...
`
      ...
      if (!transformed.includes(busyText)) {
        throw new Error('[hair-fur-v2] Busy-stage text anchor was not found')
      }
      transformed = transformed.split(busyText).join("stage === 'hair-fur' ? getHairFurText(lang).working : (stage === 'precision' ? t.precisionWorking : (stage === 'preparing' ? t.preparing : t.processing))")
...
export default defineConfig({
  ...baseConfig,
  plugins: [...(baseConfig.plugins || []), hairFurPrecisionV2()],
})
