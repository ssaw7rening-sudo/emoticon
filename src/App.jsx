...
function App() {
  ...
  const getGeminiCharacterDetails = () => {
    ...
    const photoAppearanceKo = {
      exact: '참고 사진 속 인물의 실물 이목구비(실제 눈매 각도, 무쌍/쌍꺼풀, 콧대, 입술 두께, 턱선, 헤어스타일, 피부톤)와 실사 얼굴 질감을 100% 동일하게 사실적으로 보존하여 사진 속 인물 본인임을 단번에 알아볼 수 있게 표현 (사진에 없는 안경/모자 등 임의 추가 금지)',
...
${textExclusion} No watermark, frame, duplicate character, extra limbs, cropped body, 3D textures, realistic shading, glossy gradients, complex scenery, or photorealistic background.`;
    ...
    if (lang === 'ko') {
      ...
      const textPolicy = gptTextMode === 'text'
        ? '각 셀에 지정된 문구를 캐릭터 옆이나 위에 정확히 한 번만 적으세요. 생성 전에 15개 문구의 철자와 띄어쓰기를 확인하고, 모든 글자에 선명한 흰색 스티커 외곽선(White Die-Cut Outline)을 두른 읽기 쉽고 산뜻한 2D 볼드 손글씨 폰트로 표현하세요. 텍스트 상자(박스), 말풍선, 괄호, 따옴표, 셀 번호는 절대로 넣지 마세요.'
