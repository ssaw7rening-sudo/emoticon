...
function sourceSafePipelineV22() {
  return {
    name: 'source-safe-pipeline-v22',
    enforce: 'post',
    transform(code, id) {
      ...
      if (normalizedId.endsWith('/src/components/BackgroundRemover.jsx')) {
        ...
        if (splitStart < 0 || splitEnd < 0 || splitEnd <= splitStart) {
          throw new Error('[source-safe-v22] split function boundaries not found')
        }
  ...
  workCanvas = drawn.canvas;
  workCtx = drawn.ctx;
  const width = workCanvas.width;
  const height = workCanvas.height;
  if (!width || !height) throw new Error('Invalid canvas dimensions');

  // Dark sticker sheets are rebuilt from the ORIGINAL upload. This is the only
  // authoritative source for pale/white artwork because semantic mattes can
  // already contain alpha=0 holes by the time auto-split starts.
  if (sourceFile) {
    try {
      ...
      if (alignedCtx) {
        ...
        const border = [];
        const sampleStep = Math.max(1, Math.floor(Math.min(width, height) / 520));
        const push = (x, y) => {
          const p = (y * width + x) * 4;
        ...
        if (border.length && dark.length / border.length >= 0.48) {
          ...
          if (removedRatio >= 0.08 && removedRatio <= 0.94) {
            for (let index = 0; index < total; index += 1) {
              original[index * 4 + 3] = visited[index] ? 0 : 255;
            }
      ...
      console.warn('v22 original dark-sheet rebuild skipped:', sourceError);
    }
  }
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      ...
      // Exact RGBA byte copy. No drawImage, colour key, semantic mask or alpha
      // correction is allowed after the original dark matte has been rebuilt.
      ...
      items.push({
        ...
        splitEngine: darkOriginal ? 'SRC22' : 'STD22',
        needsReview: false,
        reviewReasons: []
      });
    }
...
`
        ...
        // The source file must always be available to the splitter so dark
        // sheets can be rebuilt from untouched RGB before any individual crop.
        transformed = transformed.replaceAll('splitIntoFifteen(resultBlob);', 'splitIntoFifteen(resultBlob, file);')
        transformed = transformed.replaceAll('splitIntoFifteen(resultBlob, file);', 'splitIntoFifteen(resultBlob, file);')
        transformed = transformed.replace(/Alpha v7/g, 'Split v22 · Source Pipeline')
        transformed = transformed.replace(/Split v\d+(?: · [^`'\"}]+)?/g, 'Split v22 · Source Pipeline')
      ...
      if (normalizedId.endsWith('/src/components/EmoticonPostProcessor.jsx')) {
        ...
        if (insertAt < 0) throw new Error('[source-safe-v22] makeOutput anchor not found')
...
`
        ...
        if (count < 3) throw new Error('[source-safe-v22] expected export call sites not found')
...
export default defineConfig({
  ...baseConfig,
  plugins: [...(baseConfig.plugins || []), sourceSafePipelineV22()]
})
