...
// Replace the entire InfoSection definition in content
const startIdx = content.indexOf('const InfoSection = ({ t, lang }) => {');
const endIdx = content.indexOf('export default App;', startIdx);

if (startIdx !== -1 && endIdx !== -1) {
  content = content.slice(0, startIdx) + newInfoSectionCode + '\n\n' + content.slice(endIdx);
  fs.writeFileSync(filePath, content, 'utf8');
  console.log('Successfully updated InfoSection with full Japanese and Chinese translations!');
} else {
  console.error('Could not find startIdx or endIdx for InfoSection');
}
