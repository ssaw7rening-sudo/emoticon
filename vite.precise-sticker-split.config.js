...
function preciseStickerSheetSplit() {
  return {
    name: 'precise-sticker-sheet-split-v4',
    enforce: 'pre',
    transform(code, id) {
  ...
  // Prompt Maker's 15-emoticon sheet specification is fixed at 5 columns × 3 rows.
  // Do not infer orientation from canvas aspect ratio: image generators may return
  // a portrait or square canvas while the actual sticker arrangement is still 5×3.
  // Aspect-based switching was the cause of whole characters and captions being
  // sliced into neighboring outputs.
...
async function splitIntoFifteen(blob) {
  ...
  for (let row = 0; row < layout.rows; row += 1) {
    for (let col = 0; col < layout.cols; col += 1) {
      ...
      const padding = Math.max(8, Math.round(Math.min(cellWidth, cellHeight) * 0.07));
