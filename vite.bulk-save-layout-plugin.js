...
export function bulkSaveLayoutPlugin() {
  return {
    ...
    transform(code, id) {
      ...
      out = replaceOnce(out, titleMarker, titleReplacement, 'desktop bulk save');

      const controlMarker = `      <div className=\"mt-2.5 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap\">\n        <button type=\"button\" disabled={working} onClick={normalizeAll} className=\"min-h-11 rounded-xl border border-[#BFCDBA] bg-[#F3F8F0] px-2.5 py-2 text-xs font-extrabold leading-5 text-[#4F684A] disabled:opacity-50 sm:px-3\">▣ {working ? t.working : normalizeLabel}</button>\n        <button type=\"button\" disabled={working} onClick={downloadZip} className=\"min-h-11 rounded-xl bg-[#3E6B4B] px-2.5 py-2 text-xs font-extrabold leading-5 text-white shadow-sm disabled:opacity-50 sm:px-3\">📦 {working ? t.working : t.zip}</button>\n      </div>`;
      const controlReplacement = `      <div className=\"mt-2.5 flex flex-wrap gap-2\">\n        <button type=\"button\" disabled={working} onClick={normalizeAll} className=\"min-h-11 flex-1 rounded-xl border border-[#BFCDBA] bg-[#F3F8F0] px-2.5 py-2 text-xs font-extrabold leading-5 text-[#4F684A] disabled:opacity-50 sm:flex-none sm:px-3\">▣ {working ? t.working : normalizeLabel}</button>\n      </div>`;
      out = replaceOnce(out, controlMarker, controlReplacement, 'old bulk save control');
